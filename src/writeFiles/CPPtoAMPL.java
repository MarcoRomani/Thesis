...
public class CPPtoAMPL {
	...
	public void writeText(DataCenter dc, ArrayList<Customer> cust, ArrayList<Customer> newcust, int seed) {
		...
		for (Pod p : dc.getPods()) {
			for (Rack r : p.getRacks()) {
				for (Server s : r.getHosts()) {
					lines.add(s.getId() + "");
					ln = "";
					ln = ln + s.getCpu() + /* " "+s.getMem()+" "+s.getDisk()+" "+s.getBdw_out()+" "+s.getBdw_in()+ */" "
							+ s.getResidual_cpu() + " " + s.getResidual_mem() + " " + s.getResidual_disk() + " "
							+ s.getResidual_bdw_out() + " " + s.getResidual_bdw_in() + " " + s.getFrequency();
					lines.add(ln);
				}
			}
		}
		...
		try {
			Files.write(Paths.get("CPP_seed" + seed + "pod" + dc.getDim() + "_newC" + count + ".txt"), lines, utf8,
