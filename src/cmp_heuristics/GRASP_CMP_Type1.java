...
public class GRASP_CMP_Type1 extends GRASP_CMP_Scheme {
	...
	protected CMPSolution cluster_rand_constr(CMPSolution incumbent, List<Container> cluster, double alfa, List<Container> rest) {
	...
	protected CMPSolution inner_cpp(CMPSolution sol, Rack r, List<Container> cluster, double alfa, List<Container> rest) {
		...
		for (int iter = 0; iter < inner_cpp_iter; iter++) {
			...
			ArrayList<Container> my_rest = new ArrayList<Container>();
			CMPSolution copy = (CMPSolution)sol.clone(); // update del clone()
			vms.clear();
			vms.addAll(cluster);
			ArrayList<Double> costs = new ArrayList<Double>();
			ArrayList<ServerStub> RCL = new ArrayList<ServerStub>();
